...
| 카테고리 | 설명 | 예시 |
|----------|------|------|
| `genre` | 장르 (16개) | 판타지, 로맨스, BL, 무협, SF, 호러 |
| `setting` | 배경 (15개) | 현대, 이세계, 학원, 궁중, 타워/헌터, 연예계 |
| `protagonist` | 주인공 유형 (11개) | 회귀, 환생, 먼치킨, 악역, 귀족 |
| `character` | 캐릭터 성격 (19개) | 집착캐, 다정캐, 츤데레, 계략캐, 순정캐 |
| `tone` | 분위기 (8개) | 코믹, 다크, 달달, 진지, 사이다 |
| `theme` | 테마 (24개) | 복수, 성장, 계약, 오해, 동료케미 |

**추출 소스 및 confidence**:

| 소스 | confidence | 설명 |
|------|-----------|------|
| Ridi 키워드 정확 일치 | 0.95 | 리디북스 키워드와 정확히 매칭 |
| Ridi 키워드 부분 일치 | 0.85 | 리디북스 키워드에 포함 |
| Ridi 설명 매칭 | 0.40 ~ 0.75 | 작품 소개 텍스트에서 키워드 발견 (매치 수에 비례) |
| Enrichment 태그 정확 일치 | 0.90 | `tags` 자연어가 키워드와 정확 매칭 |
| Enrichment 태그 부분 일치 | 0.80 | `tags` 자연어가 키워드에 포함 |
| Enrichment negative 태그 | 0.0 | `negative_tags`로 명시적 제외 → refiner에서 reject |

**Negative 태그 처리**: `negative_tags`에 포함된 feature는 다른 소스에서 positive로 추출되더라도 `confidence: 0`으로 덮어씌워 refiner에서 reject된다.
...
**필터링 기준**:
- `minConfidence = 0.5` — 신뢰도 0.5 미만 후보 제거
- `tooCommonFeatures` — 너무 일반적인 피처 블랙리스트
- `ambiguousFeatures` — 모호한 피처 블랙리스트

**결과**:
- 수락된 피처 → 다음 단계(publish)로 전달
- 거절된 피처 → `refined_work_feature_rejections`에 사유와 함께 저장 (`low_confidence`, `too_common`, `ambiguous`)
